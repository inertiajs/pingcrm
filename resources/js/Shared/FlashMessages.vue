<template>
  <div>
    <div v-if="page?.props?.value?.flash?.success && show" class="flex items-center justify-between mb-8 max-w-3xl bg-green-500 rounded">
      <div class="flex items-center">
        ...
        <div class="py-4 text-white text-sm font-medium">check: {{ page?.props.value.flash.success }}</div>
      </div>
    ...
    <div v-if="(page?.props?.value?.flash?.error || Object.keys(page.props.value.errors).length > 0) && show" class="flex items-center justify-between mb-8 max-w-3xl bg-red-500 rounded">
      <div class="flex items-center">
        ...
        <div v-if="page?.props?.value?.flash?.error" class="py-4 text-white text-sm font-medium">{{ page?.props?.value?.flash?.error }}</div>
        <div v-else class="py-4 text-white text-sm font-medium">
          <span v-if="Object.keys(page.props.value.errors).length === 1">There is one form error.</span>
          <span v-else>There are {{ Object.keys(page.props.value.errors).length }} form errors.</span>
        </div>
      </div>
      <button type="button" class="group mr-2 p-2" @click="show = false">
