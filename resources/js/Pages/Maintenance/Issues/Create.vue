<template>
  <div>
    ...
    <h1 class="mb-8 text-3xl font-bold">
      <Link class="text-indigo-400 hover:text-indigo-600" href="/pannes">Pannes</Link>
    ...
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="store">
        <div class="flex flex-wrap -mb-8 -mr-6 p-8">
          <!-- Intervention Date -->
          <text-input v-model="form.intervention_date" :error="form.errors.intervention_date" class="pb-8 pr-6 w-full lg:w-1/2" label="Date de l'incident" type="date" :value="new Date()" />
          <div class="lg:w-1/2 hidden lg:block" />
          <!-- From -->
          <text-input v-model="form.from" :error="form.errors.from" class="pb-8 pr-6 w-full lg:w-1/2" label="Heure d'incident" type="time" />
          <!-- To -->
          <text-input v-model="form.to" :error="form.errors.to" class="pb-8 pr-6 w-full lg:w-1/2" label="Heure Estimée de résolution" type="time" />

          <div class="w-full border-b mb-3" />
          <div class="w-full mt-8 mb-6">
            <span class="font-bold">Zones concernées par l'Incident:</span>
          </div>
          ...
          <!-- Description -->
          <textarea-input v-model="form.description" :error="form.errors.description" class="pb-8 pr-6 w-full" label="Détails sur la panne" />
        </div>
        <div class="flex items-center justify-end px-8 py-4 bg-gray-50 border-t border-gray-100">
          <loading-button :loading="form.processing" class="btn-indigo" type="submit">Ajouter Panne</loading-button>
        </div>
      </form>
    </div>
  </div>
