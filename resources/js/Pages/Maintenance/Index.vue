<template>
  <div>
    <Head title="Travaux Planifiés" />
    <h1 class="mb-8 text-3xl font-bold">Travaux Planifiés</h1>
    <div class="flex items-center justify-between mb-6">
      <search-filter v-model="form.search" :has-filter="false" class="mr-4 w-full max-w-md" @reset="reset" />
      <Link class="btn-indigo" href="/maintenances/create">
        <span>Ajouter</span>
        <span class="hidden md:inline">&nbsp;Travail Planifié</span>
      </Link>
    </div>
      ...
      <table class="w-full whitespace-nowrap">
        <thead>
          <tr class="text-left font-bold">
            <th class="pb-4 pt-6 px-6">Date d'Intervention</th>
            <th class="pb-4 pt-6 px-6">Heure de début</th>
            <th class="pb-4 pt-6 px-6">Heure de fin</th>
  ...
  watch: {
    form: {
      ...
      handler: throttle(function () {
        this.$inertia.get('/maintenances', pickBy(this.form), { preserveState: true })
      }, 150),
    },
  },
  methods: {
