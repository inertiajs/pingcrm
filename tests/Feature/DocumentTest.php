...
class DocumentsTest extends TestCase
{
    ...
    public function test_can_search_for_documents()
    {
        ...
        $this->actingAs($this->user)
            ...
            ->assertPropValue('documents.data', function ($documents) {
                $this->assertEquals('Greg Andersson', $documents[0]['custmer_name']);
            });
    }
    ...
    {
        $document =  $this->user->account->clients()->saveMany(
            factory(Document::class, 5)->make()
        )->first();
        ...
        $this->actingAs($this->user)
            ...
            ->assertStatus(200)
            ->assertPropValue('document.custmer_name', $document->custmer_name)
